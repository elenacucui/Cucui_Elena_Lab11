...
namespace PersonalAgenda.Data
{
    public class AgendaDatabase
    {
        ...
        {
            return _database.QueryAsync<Activity>("select A.ID, A.Description from Activity A" + " inner join NoteActivity NA" + " on A.ID = NA.ActivityID where NA.AgendaID = ?", agendaid);
        }
        ...
        {
            return _database.Table<Agenda>().Where(i => i.ID == id).FirstOrDefaultAsync(); 
        }
